...
class CLLMTrainer(BaseTrainer):
    def __init__(self, args, model, train_loader, val_loader, test_loader, export_root, use_wandb, augmentor):
    ...
    def calculate_loss(self, batch):
        ...
        if self.args.dataset_code != 'xlong':
            ...
            logits, labels_, cl_loss = self.model(seqs, labels=labels)
            logits = logits.view(-1, logits.size(-1))
            labels_[labels==0] = 0
