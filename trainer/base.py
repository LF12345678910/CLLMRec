from model import *
from config import *
from utils import augmentor as augmentorPy
from utils.GRU4Rec_withNeg_Dist import GRU4Rec_withNeg_Dist
from .utils import *
from .loggers import *
...
from model.SASRec import SASRec_Model
...
class BaseTrainer(metaclass=ABCMeta):
    ...
    def train_one_epoch(self, epoch, accum_iter):
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            ...
            pos = torch.arange(seq_len).unsqueeze(0).expand(batch_size, -1).cuda()
            item_num = 3706
            sasrec = SASRec_Model(item_num, 64, seq_len, 2, 1, 0.2).cuda()
            aug_rate = 0.7
            call_count = 1
            weight = 100
            aug_seq1, aug_pos1, aug_len1 = SequenceAugmentor.augview(seqs, seq_len, aug_rate, call_count, weight)
            aug_seq2, aug_pos2, aug_len2 = SequenceAugmentor.augview(seqs, seq_len, aug_rate, call_count, weight)
            aug_seq1 = torch.tensor(aug_seq1).to('cuda')
            aug_pos1 = torch.tensor(aug_pos1).to('cuda')
            aug_emb1 = sasrec(aug_seq1, aug_pos1)
            aug_emb2 = sasrec(aug_seq2, aug_pos2)
            emb_size = 64
            cl_rate = 0.5
            cl_emb1 = [aug_emb1[i, last - 1, :].view(-1, emb_size) for i, last in enumerate(aug_len1)]
            cl_emb2 = [aug_emb2[i, last - 1, :].view(-1, emb_size) for i, last in enumerate(aug_len2)]
            cl_loss = cl_rate * InfoNCE(torch.cat(cl_emb1, 0), torch.cat(cl_emb2, 0), 1, True)
            self.optimizer.zero_grad()
            loss = self.calculate_loss(batch) + cl_loss
            loss.backward()
