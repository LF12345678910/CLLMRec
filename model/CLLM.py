...
from utils.loss_torch import l2_reg_loss,InfoNCE,batch_softmax_loss
...
class CLLM(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.embedding = CLLMEmbedding(self.args)
        self.model = CLLModel(self.args)
...
class CLLModel(nn.Module):
    def __init__(self, args):
        ...
        self.lru_blocks = nn.ModuleList([CLLMBlock(self.args) for _ in range(layers)])
    ...
    def forward(self, x, embedding_weight, mask, labels=None):
        ...
        if self.args.dataset_code != 'xlong':
            ...
            if self.training:
                ...
                labels_ = (torch.ones(labels.shape).long() * num_samples).to(labels.device)
                return scores, labels_.reshape(labels.shape), cl_loss
            

class CLLMBlock(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        hidden_size = args.bert_hidden_units
        self.lru_layer = CLLMLayer(
